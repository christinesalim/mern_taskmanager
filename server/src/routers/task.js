...
router.post('/tasks', checkAuth, async (req, res) => {
  console.log("Got post to /tasks body: ", req.body);
  ...
  try {
    ...
    console.log("Error in /tasks route");
...
router.get('/tasks', checkAuth, async (req, res) => {
  console.log('In route for GET /tasks');
  try {
...
router.patch('/tasks/:id', checkAuth, async (req, res) => {
  console.log("Received task patch request for id", req.params.id, req.body);
  ...
  try {
    ...
    console.log("Saved the task to the database successfully");
    res.send(task);
